...
async def _migrate(dry_run: bool) -> dict[str, int]:
    ...
    if OLD_TASK_DB.exists():
        ...
        print(f"  Đọc hàng đợi tác vụ cũ: tasks={len(tasks)}, events={len(events)}, leases={len(leases)}")
    else:
        print(f"  Bỏ qua (không tồn tại): {OLD_TASK_DB}")

    api_calls = []
    if OLD_USAGE_DB.exists():
        with sqlite3.connect(OLD_USAGE_DB) as conn:
            api_calls = _read_old_usage(conn)
        print(f"  Đọc dữ liệu sử dụng API cũ: api_calls={len(api_calls)}")
    else:
        print(f"  Bỏ qua (không tồn tại): {OLD_USAGE_DB}")

    sessions = []
    if OLD_SESSIONS_DB.exists():
        with sqlite3.connect(OLD_SESSIONS_DB) as conn:
            sessions = _read_old_sessions(conn)
        print(f"  Đọc phiên cũ: sessions={len(sessions)}")
    else:
        print(f"  Bỏ qua (không tồn tại): {OLD_SESSIONS_DB}")

    if dry_run:
        print("\n[DRY RUN] Không ghi DB, không đổi tên tệp cũ.")
        return {
            "tasks": len(tasks),
            "events": len(events),
            "leases": len(leases),
    ...
    for old_path in [OLD_TASK_DB, OLD_USAGE_DB, OLD_SESSIONS_DB]:
        if old_path.exists():
            ...
            print(f"  Đã đổi tên: {old_path.name} → {bak_path.name}")
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Chỉ đọc, không ghi")
    args = parser.parse_args()

    print("=== Di chuyển dữ liệu SQLite → ORM ===\n")
    print(f"Chế độ: {'DRY RUN' if args.dry_run else 'Di chuyển thực tế'}\n")

    stats = asyncio.run(_migrate(args.dry_run))

    print("\n=== Thống kê di chuyển ===")
    for key, count in stats.items():
        print(f"  {key}: {count} bản ghi")
