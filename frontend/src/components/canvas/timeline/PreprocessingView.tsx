...
export function PreprocessingView({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setEditing(false);
      pushToast("Đã lưu nội dung tiền xử lý", "success");
    } catch {
      pushToast("Lưu thất bại", "error");
    } finally {
      setSaving(false);
    }
  ...
  const statusLabel =
    contentMode === "narration" ? "Tách phân đoạn đã hoàn tất" : "Chuẩn hóa kịch bản đã hoàn tất";

  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center text-gray-500">
        Đang tải nội dung tiền xử lý...
      </div>
    );
  }
