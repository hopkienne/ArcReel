...
export function VersionTimeMachine({
  ...
  async function handleRestore(version: number) {
    ...
    try {
      ...
      useAppStore.getState().pushToast(`Đã chuyển sang v${version}`, "success");
    } catch (err) {
      useAppStore
        .getState()
        .pushToast(`Chuyển phiên bản thất bại: ${(err as Error).message}`, "error");
    } finally {
      setRestoringVersion(null);
    }
  }
          ...
          >
            {loading ? (
              ...
              <div className="space-y-1">
                <p className="text-[11px] font-medium text-gray-300">Chưa có lịch sử phiên bản</p>
                <p className="text-[11px] leading-5 text-gray-500">
                  Sau khi tạo mới hoặc khôi phục, lịch sử phiên bản sẽ xuất hiện tại đây.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                {!selectedInfo && (
                  <p className="text-[10px] leading-4 text-gray-400">
                    Bấm vào phiên bản để xem trước; có thể chuyển sang phiên bản không phải hiện tại.
                  </p>
                )}
                  ...
                  <div className="rounded-xl border border-gray-700 bg-gray-950/80 p-2.5">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      ...
                      {selectedInfo.is_current ? (
                        ...
                        >
                          {restoringVersion === selectedInfo.version ? "Đang chuyển..." : "Chuyển sang phiên bản này"}
                        </button>
                      )}
                    </div>
                    ...
                    {selectedInfo.file_url &&
                      (resourceType === "videos" ? (
                        ...
                        >
                          <img
                            src={selectedInfo.file_url}
                            alt={`Xem trước phiên bản v${selectedInfo.version}`}
                    ...
                    <p className="line-clamp-4 text-[11px] leading-5 text-gray-400">
                      {selectedInfo.prompt || "Phiên bản này không có ghi chú bổ sung."}
                    </p>
