...
export function TimelineCanvas({
  ...
  if (!projectData || (!episodeScript && !hasDraft)) {
    return (
      <div className="flex h-full items-center justify-center text-gray-500">
        Vui lòng chọn tập ở thanh bên trái
      </div>
    );
  }
  ...
  const segmentLabel = contentMode === "narration" ? "phân đoạn" : "cảnh";
        ...
        <div className="mb-4">
          ...
          {episodeCost && (
            <div className="mt-2 flex items-center gap-4 rounded-lg bg-gray-900 border border-gray-800 px-3 py-2 text-xs tabular-nums">
              ...
              <span className="text-gray-500">Tổng <span className="font-medium text-amber-400">{formatCost(totalBreakdown(episodeCost.totals.estimate))}</span></span>
              ...
              <span className="text-gray-500">Tổng <span className="font-medium text-emerald-400">{formatCost(totalBreakdown(episodeCost.totals.actual))}</span></span>
            </div>
          )}
        </div>
