...
export function OverviewCanvas({ projectName, projectData }: OverviewCanvasProps) {
  ...
  const handleUpload = useCallback(
    async (file: File) => {
      ...
      useAppStore.getState().pushToast(`Tệp nguồn "${file.name}" đã tải lên thành công`, "success");
    },
    [projectName],
  );
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    try {
      await API.generateOverview(projectName);
      await refreshProject();
      useAppStore.getState().pushToast("Đã tạo lại tổng quan dự án", "success");
    } catch (err) {
      useAppStore
        .getState()
  ...
  const handleStyleImageChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        await refreshProject();
        useAppStore.getState().pushToast("Đã cập nhật ảnh tham chiếu phong cách", "success");
      } catch (err) {
        useAppStore
          .getState()
  ...
  const handleDeleteStyleImage = useCallback(async () => {
    ...
    try {
      ...
      await refreshProject();
      useAppStore.getState().pushToast("Đã xóa ảnh tham chiếu phong cách", "success");
    } catch (err) {
      useAppStore
        .getState()
  ...
  const handleSaveStyleDescription = useCallback(async () => {
    ...
    try {
      ...
      await refreshProject();
      useAppStore.getState().pushToast("Đã lưu mô tả phong cách", "success");
    } catch (err) {
      useAppStore
        .getState()
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="max-w-2xl text-xs leading-5 text-gray-500">
            Ảnh tham chiếu sẽ được dùng trong các lần tạo hình ảnh tiếp theo; mô tả phong cách giúp bổ sung quy tắc thị giác, cân chỉnh tông màu, chất liệu và ngôn ngữ máy quay.
          </p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
        <div className="space-y-3">
          {styleImageUrl ? (
            ...
            >
              ...
              <span>{uploadingStyleImage ? "Đang tải lên..." : "Tải ảnh tham chiếu phong cách"}</span>
          ...
          <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-3">
            <p className="text-xs font-medium text-gray-400">Hướng dẫn</p>
            <p className="mt-1 text-sm leading-6 text-gray-300">
              {styleImageUrl
                ? "Ảnh tham chiếu hiện tại sẽ là chuẩn thị giác thống nhất cho ảnh nhân vật, storyboard và tạo video."
                : "Hiện chưa có ảnh tham chiếu cấp dự án, bạn có thể tải lên một ảnh mẫu phong cách mục tiêu làm chuẩn."}
            </p>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              >
                ...
                {styleImageUrl ? "Thay ảnh tham chiếu" : "Tải ảnh tham chiếu"}
              </button>
              {styleImageUrl && (
                <button
                  type="button"
          ...
          <input
            ...
            className="hidden"
            aria-label="Tải ảnh tham chiếu phong cách"
          />
        </div>
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-950/35 p-4">
          ...
          <p className="mt-1 text-xs leading-5 text-gray-500">
            Sau khi tải ảnh tham chiếu, hệ thống sẽ tự phân tích và điền mô tả phong cách; bạn cũng có thể tinh chỉnh thủ công.
          </p>
          ...
          <textarea
            ...
            placeholder="Sau khi tải ảnh tham chiếu phong cách, hệ thống sẽ tự phân tích và điền mô tả; bạn cũng có thể chỉnh sửa thủ công."
          />

          <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
            <p className="text-xs leading-5 text-gray-500">
              {styleImageUrl
                ? "Nên dùng mô tả phong cách để bổ sung ánh sáng, màu sắc, chất liệu và ngôn ngữ máy quay."
                : "Ngay cả khi chưa có ảnh tham chiếu, bạn vẫn có thể mô tả rõ phong cách hình ảnh và tiêu chí thẩm mỹ bằng văn bản."}
            </p>
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">
            {projectData.content_mode === "narration"
              ? "Chế độ kể chuyện + hình ảnh"
              : "Chế độ hoạt hình theo tập"}{" "}
            · {projectData.style || "Chưa đặt phong cách"}
          </p>
        </div>
          ...
          <>
            {overview && (
              <div className="space-y-3 rounded-xl border border-gray-800 bg-gray-900 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-gray-300">Tổng quan dự án</h3>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    />
                    <span>{regenerating ? "Đang tạo..." : "Tạo lại"}</span>
                  </button>
                </div>
                <p className="text-sm text-gray-400">{overview.synopsis}</p>
                <div className="flex gap-4 text-xs text-gray-500">
                  <span>Thể loại: {overview.genre}</span>
                  <span>Chủ đề: {overview.theme}</span>
                </div>
              </div>
            )}
            ...
            {costLoading && (
              <div className="rounded-xl border border-gray-800 bg-gray-900 p-4">
                <p className="text-sm text-gray-500 animate-pulse">Đang tính chi phí...</p>
              </div>
            )}
            {costError && (
              <div className="rounded-xl border border-red-900/50 bg-red-950/30 p-4">
                <p className="text-sm text-red-400">Ước tính chi phí thất bại: {costError}</p>
              </div>
            )}

            {projectTotals && (
              <div className="rounded-xl border border-gray-800 bg-gray-900 p-4 tabular-nums">
                <p className="mb-3 text-sm font-semibold text-gray-300">Tổng chi phí dự án</p>
                <dl className="flex flex-wrap items-start justify-between gap-6">
                  <div className="min-w-0">
                    <dt className="mb-1 text-[11px] text-gray-600">Ước tính</dt>
                    <dd className="text-sm text-gray-400">
                      ...
                      <span className="ml-3 text-gray-500">Tổng </span>
                  ...
                  <div className="min-w-0">
                    <dt className="mb-1 text-[11px] text-gray-600">Thực tế</dt>
                    <dd className="text-sm text-gray-400">
                      <span className="text-gray-500">Storyboard </span>
                      ...
                      {projectTotals.actual.character_and_clue && (
                        <>
                          <span className="ml-3 text-gray-500">Nhân vật/Manh mối </span>
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-gray-300">Các tập</h3>
              {(projectData.episodes?.length ?? 0) === 0 ? (
                <p className="text-sm text-gray-500">
                  Chưa có tập nào. Hãy dùng trợ lý AI để tạo kịch bản.
                </p>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-gray-500">
                        {ep.scenes_count ?? "?"} phân đoạn · {ep.status ?? "draft"}
                      </span>
                      {epCost && (
                        <span className="ml-auto flex min-w-0 flex-shrink flex-wrap gap-4 text-xs text-gray-400">
                          <span>
                            ...
                            <span className="ml-2 text-gray-500">Tổng </span><span className="font-medium text-amber-400">{formatCost(totalBreakdown(epCost.totals.estimate))}</span>
                          </span>
                          ...
                          <span>
                            ...
                            <span className="ml-2 text-gray-500">Tổng </span><span className="font-medium text-emerald-400">{formatCost(totalBreakdown(epCost.totals.actual))}</span>
                          </span>
                        </span>
                      )}
                    </div>
