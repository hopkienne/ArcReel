...
describe("AddCharacterForm", () => {
  ...
  it("submits an optional reference file together with the new character", async () => {
    ...
    fireEvent.change(
      screen.getByPlaceholderText("Mô tả ngoại hình, tính cách, bối cảnh của nhân vật..."),
      {
        target: { value: "hero desc" },
      },
    );
    fireEvent.change(screen.getByPlaceholderText("Tùy chọn, ví dụ: dịu dàng nhưng uy nghiêm"), {
      target: { value: "warm" },
    });
