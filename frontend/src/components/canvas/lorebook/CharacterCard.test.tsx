...
describe("CharacterCard", () => {
  ...
  it("renders existing saved reference image", () => {
    ...
    expect(screen.getByAltText("Hero ảnh tham chiếu")).toHaveAttribute(
      "src",
  ...
  it("keeps selected reference file until save and submits it in the payload", async () => {
    ...
    expect(screen.getByText("Ảnh tham chiếu chờ lưu")).toBeInTheDocument();
  ...
  it("auto-resizes the description textarea as content grows", async () => {
    ...
    const textarea = screen.getByPlaceholderText("Nhập mô tả nhân vật...");
