            ...
            {sessions.map((session) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); if (confirm("Bạn có chắc muốn xóa phiên này không? Không thể hoàn tác.")) onDelete(session.id); }}
...
export function AgentCopilot() {
  ...
  const inputPlaceholder = pendingQuestion
    ? "Hãy trả lời câu hỏi bên trên trước"
    : isRunning
      ? "Trợ lý đang sinh nội dung, bạn có thể bấm dừng"
      : "Nhập tin nhắn, gõ / để xem kỹ năng";
  ...
  const addImages = useCallback((files: File[]) => {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_IMAGE_BYTES) {
        setAttachError(`Ảnh "${file.name}" vượt quá 5MB, đã bỏ qua`);
        continue;
      }
      ...
      <div className="flex h-10 items-center justify-between border-b border-gray-800 px-3">
        <div className="flex items-center gap-2">
          <button
            ...
            title="Thu gọn panel trợ lý"
          >
            <PanelRightClose className="h-4 w-4" />
          </button>
          <Bot className="h-4 w-4 text-indigo-400" />
          <span className="text-sm font-medium text-gray-300">Trợ lý ArcReel</span>
        </div>
      ...
      <div className="border-t border-gray-800 p-3">
            ...
            {attachedImages.map((img) => (
              <div key={img.id} className="relative">
                <button
                  ...
                  aria-label="Bấm để phóng to ảnh"
                >
                  <img
                    src={img.dataUrl}
        ...
        >
          ...
          <button
            ...
            title={attachedImages.length >= MAX_IMAGES ? `Tối đa đính kèm ${MAX_IMAGES} ảnh` : "Đính kèm ảnh"}
