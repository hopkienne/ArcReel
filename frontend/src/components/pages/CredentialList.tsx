...
const CredentialRow = memo(function CredentialRow({ cred, providerId, isVertex, onChanged }: RowProps) {
  ...
  const handleActivate = useCallback(async () => {
    try {
      ...
      // Lỗi mạng được xử lý im lặng, người dùng có thể thử lại
    }
    ...
    >
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label={cred.is_active ? "Đang sử dụng" : `Kích hoạt ${cred.name}`}
        ...
        >
          ...
          {testResult.success && testResult.available_models.length > 0 && (
            <div className="mt-1 opacity-70">
              Mô hình khả dụng: {testResult.available_models.join(", ")}
            </div>
          )}
        </div>
      )}
      ...
      {editing && (
        <div className="mt-2.5 ml-5.5 space-y-2.5 rounded-lg border border-gray-800 bg-gray-950/60 p-3">
          ...
          <div>
            <label htmlFor={`${editPrefix}-apikey`} className="mb-1 block text-xs text-gray-500">API Key (để trống để giữ giá trị hiện tại)</label>
            <input
              ...
              placeholder="Để trống để giữ giá trị hiện tại…"
          ...
          {providerId === "gemini-aistudio" && (
            <div>
              ...
              <input
                ...
                placeholder="Mặc định dùng địa chỉ chính thức…"
                className={inputClsPlaceholder}
              />
            </div>
          )}
...
function AddCredentialForm({ providerId, isVertex, onCreated, onCancel }: AddFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isVertex) {
        ...
        if (!file) {
          setError("Vui lòng chọn tệp thông tin xác thực");
          setSaving(false);
          return;
        }
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-950/60 p-3 space-y-2.5">
      ...
      {isVertex ? (
        <div>
          <label htmlFor="cred-add-file" className="mb-1 block text-xs text-gray-500">Tệp thông tin xác thực <span className="text-rose-400">*</span></label>
          <button
            id="cred-add-file"
            type="button"
          ...
          >
            ...
            {fileRef.current?.files?.[0]?.name ?? "Chọn tệp JSON…"}
          </button>
...
export function CredentialList({ providerId, onChanged }: Props) {
  ...
  // Dùng ref để giữ onChanged ổn định, tránh vòng lặp vô hạn khi component cha re-render
  ...
  // Sau thao tác của người dùng: làm mới danh sách + thông báo cho component cha
  ...
  return (
    <div>
      <div className="mb-2.5 flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-300">Quản lý khóa</h4>
        {!showAdd && (
          <button
            type="button"
          ...
          >
            <Plus className="h-3 w-3" /> Thêm khóa
          </button>
        )}
      </div>
      ...
      {credentials.length === 0 && !showAdd && (
        <div className="rounded-lg border border-dashed border-gray-700 px-4 py-6 text-center">
          <p className="text-sm text-gray-500">Chưa có khóa</p>
          <button
            type="button"
            onClick={() => setShowAdd(true)}
          ...
          >
            <Plus className="h-3 w-3" /> Thêm khóa đầu tiên
          </button>
        </div>
      )}
