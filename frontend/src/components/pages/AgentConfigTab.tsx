...
const MODEL_ROUTING_FIELDS = [
  {
    ...
    hint: "Tác vụ nhẹ (phân loại, trích xuất, hỏi đáp đơn giản)",
  ...
  },
  {
    ...
    hint: "Tác vụ cân bằng (viết lách, điều phối, suy luận nhiều bước)",
  ...
  },
  {
    ...
    hint: "Tác vụ phức tạp (soạn nội dung dài, phân tích sâu)",
    patchKey: "anthropic_default_opus_model" as const,
  },
  {
    key: "subagentModel" as const,
    label: "Mô hình Subagent",
    envVar: "CLAUDE_CODE_SUBAGENT_MODEL",
    hint: "Mô hình dùng khi Subagent chạy song song",
    patchKey: "claude_code_subagent_model" as const,
  },
] as const;

// Nút xóa nhanh hiển thị cạnh "Hiện tại:" khi có giá trị
...
export function AgentConfigTab({ visible }: AgentConfigTabProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      useAppStore.getState().pushToast("Đã lưu cấu hình tác tử ArcReel", "success");
    } catch (err) {
  ...
  const handleClearField = useCallback(
    async (fieldId: string, patch: SystemConfigPatch, label: string) => {
      ...
      try {
        ...
        useConfigStatusStore.getState().refresh();
        useAppStore.getState().pushToast(`Đã xóa ${label}`, "success");
      } catch (err) {
      ...
      <div className="space-y-8 px-6 pb-0 pt-6">
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-100">Tác tử ArcReel</h2>
              <p className="text-sm text-gray-500">
                Dựa trên Claude Agent SDK, vận hành trợ lý AI hội thoại và quy trình tự động hóa
              </p>
            </div>
          </div>
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-gray-800/60 bg-gray-900/30 px-3 py-2">
            ...
            <p className="text-xs text-gray-500">
              Cấu hình tương thích quy ước biến môi trường của Claude Code, có thể dùng Coding Plan API tương thích Claude Code.
            </p>
          </div>
        </div>
        ...
        <div>
          <SectionHeading
            ...
            description="Khóa Anthropic API là điều kiện bắt buộc để tác tử hoạt động"
          />
          ...
          <div className={`${cardClassName} space-y-4`}>
                ...
                {settings.anthropic_api_key.is_set && (
                  <div className="flex items-center text-xs text-gray-500">
                    <span className="truncate">
                      Hiện tại: {settings.anthropic_api_key.masked ?? "Đã thiết lập"}
                    </span>
                    <button
                      type="button"
                      onClick={() =>
                      ...
                      aria-label="Xóa Anthropic API Key đã lưu"
                    >
            ...
            <div className="border-t border-gray-800 pt-4">
              <div className="flex items-center justify-between">
                ...
                {settings.anthropic_base_url && (
                  <button
                    ...
                    aria-label="Xóa Anthropic Base URL đã lưu"
                  >
              ...
              <p className="mt-0.5 text-xs text-gray-500">
                Tương ứng ANTHROPIC_BASE_URL, để trống sẽ dùng địa chỉ mặc định
              </p>
              <div className="relative mt-2">
                <input
                  id="agent-base-url"
                ...
                {draft.anthropicBaseUrl && (
                  <button
                    ...
                    className={`absolute right-2 top-1/2 -translate-y-1/2 ${smallBtnClassName}`}
                    aria-label="Xóa Base URL đã nhập"
                  >
                    <X className="h-3.5 w-3.5" />
                  </button>
                )}
        ...
        <div>
          <SectionHeading
            ...
            description="Chỉ định mô hình Claude cho tác tử. Để trống sẽ dùng mặc định của Claude Agent SDK."
          />
          ...
          <div className={cardClassName}>
            <div className="flex items-center justify-between">
              ...
              {settings.anthropic_model && (
                <button
                  ...
                  disabled={isBusy}
                  className="inline-flex items-center gap-1 rounded text-xs text-gray-600 transition-colors hover:text-rose-400 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:ring-2 focus-visible:ring-indigo-500/60 focus-visible:outline-none"
                  aria-label="Xóa cấu hình mô hình đã lưu"
                >
            ...
            <p className="mt-0.5 text-xs text-gray-500">
              Tương ứng ANTHROPIC_MODEL, dùng để ghi đè mô hình mặc định
            </p>
            <div className="relative mt-2">
              <input
                id="agent-model"
              ...
              {draft.anthropicModel && (
                <button
                  ...
                  className={`absolute right-2 top-1/2 -translate-y-1/2 ${smallBtnClassName}`}
                  aria-label="Xóa nội dung cấu hình mô hình"
                >
                  <X className="h-3.5 w-3.5" />
                </button>
              )}
            ...
            >
              ...
              <p className="mt-2 text-xs text-gray-500">
                Claude Agent SDK hỗ trợ định tuyến theo cấp độ năng lực sang các mô hình khác nhau. Để trống sẽ dùng chung mô hình mặc định phía trên.
              </p>
                      ...
                      <div className="relative mt-1.5">
                        ...
                        {draft[key] && (
                          <button
                            ...
                            className={`absolute right-2 top-1/2 -translate-y-1/2 ${smallBtnClassName}`}
                            aria-label={`Xóa nội dung nhập ${label}`}
                          >
                            <X className="h-3.5 w-3.5" />
                          </button>
                        )}
          ...
          <details>
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-500">
                  Sau khi phiên kết thúc sẽ chờ thời gian này rồi giải phóng tài nguyên, khi trò chuyện lại sẽ tự khôi phục
                </p>
                <input
                  type="number"
                  min={10}
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-500">
                  Giới hạn số phiên tác tử hoạt động đồng thời; vượt ngưỡng sẽ tự giải phóng phiên lâu không dùng nhất (phiên đã dọn vẫn được lưu để khôi phục ở lần sau)
                </p>
                <input
                  type="number"
                  min={1}
