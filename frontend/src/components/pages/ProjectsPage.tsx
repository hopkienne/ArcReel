      ...
      <div className="w-full max-w-md rounded-2xl border border-amber-400/20 bg-gray-900 p-6 shadow-2xl shadow-black/40">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            <h2 className="text-lg font-semibold text-gray-100">Phát hiện mã dự án bị trùng</h2>
            <p className="text-sm leading-6 text-gray-400">
              Mã dự án trong gói nhập
              <span className="mx-1 rounded bg-gray-800 px-1.5 py-0.5 font-mono text-gray-200">
                {projectName}
              </span>
              đã tồn tại. Bạn có thể ghi đè dự án hiện có, hoặc tự đổi tên rồi tiếp tục nhập.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3">
          ...
          >
            <span>
              ...
              <span className="mt-1 block text-xs text-red-200/80">
                Dùng dữ liệu trong gói nhập để thay thế dữ liệu của mã dự án hiện có
              </span>
            </span>
          ...
          <button
            ...
            disabled={importing}
            aria-label="Tự đổi tên khi nhập"
          ...
          >
            <span>
              <span className="block font-medium">Tự đổi tên khi nhập</span>
              <span className="mt-1 block text-xs text-indigo-200/80">
                Giữ nguyên dự án hiện có, dự án mới sẽ tự tạo mã nội bộ mới
              </span>
            </span>
            {importing && <Loader2 className="h-4 w-4 animate-spin" />}
          </button>
...
function ImportDiagnosticsDialogWrapper({
  ...
  return (
    <ArchiveDiagnosticsDialog
      ...
      description="Tiền kiểm tra nhập đã hoàn tất. Các vấn đề dưới đây được nhóm theo mức độ nghiêm trọng; vấn đề chặn cần được xử lý trước khi tiếp tục nhập."
      sections={[
    ...
    >
      ...
      {summary && summary.total > 0 && (
        <div className="text-xs text-gray-500">
          {summary.total} tập
          {summary.scripted > 0 && ` · ${summary.scripted} tập hoàn tất kịch bản`}
          {summary.in_production > 0 && ` · ${summary.in_production} tập đang sản xuất`}
          {summary.completed > 0 && ` · ${summary.completed} tập hoàn thành`}
        </div>
      )}
    </button>
  );
    ...
    async (
      ...
      try {
        ...
        useAppStore.getState().pushToast(
          autoFixedCount > 0
            ? `Đã nhập dự án "${result.project.title || result.project_name}", tự sửa ${autoFixedCount} mục`
            : `Đã nhập dự án "${result.project.title || result.project_name}"`,
          "success"
        );
        if (warningCount > 0) {
          const warningMessages = result.diagnostics.warnings.map((w) => w.message).join("；");
          useAppStore.getState().pushToast(
            `Cảnh báo khi nhập: ${warningMessages}`,
            "warning"
          );
        }
        ...
        useAppStore
          ...
          .pushToast(
            ...
            + (autoFixableCount > 0 ? `, ${autoFixableCount} mục có thể tự sửa)` : ")"),
            "error"
          );
      } finally {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-1 flex items-center gap-1 border-l border-gray-800 pl-3">
              ...
              >
                ...
                {!isConfigComplete && (<span className="absolute right-0.5 top-0.5 h-2 w-2 rounded-full bg-rose-500" aria-label="Cấu hình chưa đầy đủ" />)}
              </button>
            </div>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        {projectsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            ...
            <p className="text-sm mt-1">Bấm góc trên bên phải "Tạo dự án mới" hoặc "Nhập ZIP" để bắt đầu</p>
          </div>
        ) : (
