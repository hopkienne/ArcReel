...
function priceLabel(mediaType: MediaType): { input: string; output: string } {
  ...
  if (mediaType === "image") return { input: "/ảnh", output: "" };
  return { input: "/M đầu vào", output: "/M đầu ra" };
}
...
export function CustomProviderForm({ existing, onSaved, onCancel }: CustomProviderFormProps) {
  ...
  const handleDiscover = useCallback(async () => {
    if (!baseUrl) {
      showError("Vui lòng nhập Base URL trước");
      return;
    }
    if (!apiKey) {
      showError("Vui lòng nhập API Key trước");
      return;
    }
    setDiscovering(true);
    try {
      ...
      showError(e instanceof Error ? e.message : "Không thể lấy danh sách mô hình");
    } finally {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      setTestResult({ success: false, message: e instanceof Error ? e.message : "Kiểm tra kết nối thất bại" });
    } finally {
      setTesting(false);
    }
  ...
  const handleSave = useCallback(async () => {
    ...
    if (enabledModels.length === 0) {
      showError("Cần bật ít nhất một mô hình");
      return;
    }
    const emptyId = enabledModels.find((m) => !m.model_id.trim());
    if (emptyId) {
      showError("Các mô hình đã bật phải có model_id");
      return;
    }
    setSaving(true);
    try {
      if (isEdit && existing) {
        // Cập nhật nguyên tử provider + models trong một giao dịch
  ...
  const urlPreview = (() => {
    ...
    if (apiFormat === "openai") {
      // OpenAI SDK cần hậu tố /v1, backend sẽ tự động bổ sung
      const base = trimmed.match(/\/v\d+$/) ? trimmed : `${trimmed}/v1`;
      return `${base}/models`;
    }
    // Google SDK tự ghép /v1beta, backend sẽ loại bỏ phần phiên bản nhập sai
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <h3 className="mb-6 text-lg font-semibold text-gray-100">
        {isEdit ? "Chỉnh sửa nhà cung cấp tùy chỉnh" : "Thêm nhà cung cấp tùy chỉnh"}
      </h3>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder={isEdit ? existing?.api_key_masked ?? "Để trống để giữ API key hiện tại" : "Nhập API Key"}
          ...
          <div>
            <div className="mb-2 flex items-center gap-3 text-sm text-gray-400">
              ...
              {models.length > 1 && (
                ...
                >
                  {filteredModels.every((m) => m.is_enabled) ? "Bỏ chọn tất cả" : "Chọn tất cả"}
                </button>
              )}
            </div>
            {models.length > 5 && (
        ...
        {models.length === 0 && (
          <div className="rounded-xl border border-dashed border-gray-700 p-4 text-center text-sm text-gray-500">
            Nhấn "Lấy danh sách mô hình" để tự phát hiện, hoặc
            <button
              type="button"
              onClick={addManualModel}
