...
const percentFmt = new Intl.NumberFormat("vi-VN", { style: "percent", maximumFractionDigits: 0 });

const TIME_RANGES = [
  { label: "7 ngày gần đây", days: 7 },
  { label: "30 ngày gần đây", days: 30 },
  { label: "Tất cả", days: 0 },
];
...
export function UsageStatsSection() {
  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        <h3 className="text-lg font-semibold text-gray-100">Thống kê sử dụng</h3>
        <p className="mt-1 text-sm text-gray-500">Xem thống kê gọi API theo nhà cung cấp</p>
      </div>
          ...
          {stats.map((s) => (
            <div key={`${s.provider}-${s.call_type}`} className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-6 text-xs tabular-nums text-gray-400">
                <span>Lượt gọi: {s.total_calls}</span>
