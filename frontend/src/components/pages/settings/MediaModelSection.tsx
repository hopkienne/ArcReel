...
export function MediaModelSection() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      useAppStore.getState().pushToast("Đã lưu cấu hình mô hình phương tiện", "success");
    } catch (err) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Thiết lập mô hình mặc định toàn cục, có thể ghi đè trong từng dự án</p>
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
        ...
        {videoBackends.length > 0 ? (
          ...
          <div className="rounded-lg border border-gray-800 bg-gray-900/60 px-3 py-2 text-sm text-gray-500">
            Chưa có nhà cung cấp video khả dụng, hãy cấu hình API key ở trang "Nhà cung cấp" trước
          </div>
        )}
        ...
        <label className="mt-3 flex cursor-pointer items-center gap-2 text-sm text-gray-300">
          ...
          />
          Tạo âm thanh
          <span className="text-xs text-gray-500">(chỉ một số nhà cung cấp hỗ trợ)</span>
        </label>
      </div>

      {/* Image backend selector */}
      <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
        <div className="mb-3 text-sm font-medium text-gray-100">Mô hình hình ảnh mặc định</div>
        {imageBackends.length > 0 ? (
          ...
          <div className="rounded-lg border border-gray-800 bg-gray-900/60 px-3 py-2 text-sm text-gray-500">
            Chưa có nhà cung cấp hình ảnh khả dụng, hãy cấu hình API key ở trang "Nhà cung cấp" trước
          </div>
        )}
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-950/40 p-4">
        ...
        <p className="mb-3 text-xs text-gray-500">Cấu hình mô hình văn bản theo loại tác vụ, để trống nghĩa là tự động chọn</p>
        ...
        {textBackends.length > 0 ? (
          ...
          <div className="rounded-lg border border-gray-800 bg-gray-900/60 px-3 py-2 text-sm text-gray-500">
            Chưa có nhà cung cấp văn bản khả dụng, hãy cấu hình API key ở trang "Nhà cung cấp" trước
          </div>
        )}
      </div>
