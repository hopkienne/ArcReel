 ...
 * Hiển thị danh sách, tạo mới (modal hiển thị key đầy đủ), xóa (modal xác nhận).
 */
...
function CreateModal({ onClose, onCreated }: CreateModalProps) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    setCreating(true);
    try {
      // expiresDays === "" hoặc 0 thì gửi 0 (backend hiểu là không hết hạn).
      // Số nguyên dương truyền trực tiếp; undefined để backend dùng mặc định (30 ngày).
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-2.5 rounded-xl border border-amber-500/20 bg-amber-500/8 px-3 py-3">
                ...
                <p className="text-xs leading-5 text-amber-200">
                  Hãy sao chép và lưu API Key này ngay. Vì lý do bảo mật, khóa đầy đủ <strong className="font-semibold">chỉ hiển thị một lần khi tạo</strong>, đóng cửa sổ là không thể xem lại.
                </p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Để trống để không hết hạn"
                  className="w-full rounded-xl border border-gray-700 bg-gray-950 px-3 py-2.5 text-sm text-gray-200 placeholder:text-gray-600 focus:border-indigo-500/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40"
                />
                <p className="mt-1 text-xs text-gray-600">Mặc định 30 ngày; để trống là không bao giờ hết hạn</p>
              </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs leading-5 text-gray-400">
                ...
                Dịch vụ đang dùng Key này sẽ mất quyền truy cập ngay lập tức, thao tác này không thể hoàn tác.
              </p>
            </div>
          </div>
    ...
    <tr className="group border-t border-gray-800/70 transition-colors hover:bg-gray-800/30">
      ...
      <td className="hidden px-3 py-3 md:table-cell">
        {keyInfo.expires_at ? (
          ...
          <span className="text-xs text-gray-600">Không hết hạn</span>
        )}
      </td>
...
export function ApiKeysTab() {
  ...
  const handleDeleted = useCallback((keyId: number) => {
    ...
    useAppStore.getState().pushToast("API Key đã được thu hồi", "success");
  }, []);
      ...
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-gray-500">
            Dùng cho dịch vụ bên ngoài như OpenClaw truy cập ArcReel API bằng Bearer Token
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900/60 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-14 text-gray-600">
            ...
            <p className="text-xs">Nhấn "Tạo Key" để tạo key đầu tiên</p>
          </div>
        ) : (
