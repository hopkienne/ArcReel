...
export function AssetSidebar({ className }: AssetSidebarProps) {
  ...
  const handleUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Đặt lại input để có thể chọn lại cùng tệp
    e.target.value = "";
  ...
  const handleDeleteFile = useCallback(async (filename: string) => {
    ...
    if (!confirm(`Bạn có chắc chắn muốn xóa "${filename}" không?`)) return;
    try {
      await API.deleteSourceFile(projectName, filename);
      loadSourceFiles();
      useAppStore.getState().invalidateSourceFiles();
      // Nếu đang mở tệp này thì quay lại tổng quan
  ...
  return (
    ...
    >
      ...
      <CollapsibleSection title="Sổ tay nội dung" icon={Users} defaultOpen={true}>
      ...
      {/* ---- Section 3: Episodes ---- */}
      <CollapsibleSection title="Tập" icon={Film}>
        {episodes.length === 0 ? (
          <EmptyState text="Chưa có tập" />
        ) : (
          <ul>
            {episodes.map((ep) => {
