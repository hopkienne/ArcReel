...
/** Dùng thẻ <a> ẩn để kích hoạt tải xuống trình duyệt, tránh mở tab trống. */
...
function PhaseStepper({
  ...
  return (
    <nav className="flex items-center gap-1" aria-label="Các giai đoạn quy trình">
...
export function GlobalHeader({ onNavigateBack }: GlobalHeaderProps) {
  ...
  // Tải thống kê chi phí (tự động làm mới khi task hoàn tất)
  ...
  const modeBadgeText =
    contentMode === "drama" ? "Hoạt hình theo tập 16:9" : "Kể chuyện 9:16";
  ...
  const handleJianyingExport = async (episode: number, draftPath: string, jianyingVersion: string) => {
    ...
    try {
      ...
      useAppStore.getState().pushToast("Đã bắt đầu xuất dự thảo Jianying, hãy giải nén file ZIP vào thư mục dự thảo Jianying", "success");
    } catch (err) {
      useAppStore.getState().pushToast(`Xuất dự thảo Jianying thất bại: ${(err as Error).message}`, "error");
    } finally {
    ...
    try {
      ...
      if (diagnosticCount > 0) {
        ...
        useAppStore.getState().pushToast(
          `Đã bắt đầu tải ZIP dự án, gói xuất có ${diagnosticCount} mục chẩn đoán`,
          "warning",
        );
      } else {
    ...
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-gray-800 bg-gray-900/80 px-4 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={taskHudAnchorRef}>
          <button
            ...
            aria-label="Bật tắt bảng tác vụ"
          >
            <Activity
        ...
        >
          ...
          {!isConfigComplete && !currentProjectName && (
            <span className="absolute right-0.5 top-0.5 h-2 w-2 rounded-full bg-rose-500" aria-label="Cấu hình chưa đầy đủ" />
          )}
        </button>
      ...
      {exportDiagnostics !== null && (
        <ArchiveDiagnosticsDialog
          ...
          description="Quá trình xuất đã tiền kiểm và tạo ZIP. Các vấn đề dưới đây được phát hiện trong gói xuất."
          sections={[
            { key: "blocking", title: "Vấn đề chặn", tone: "border-red-400/25 bg-red-500/10 text-red-100", items: exportDiagnostics.blocking },
            { key: "auto_fixed", title: "Đã tự sửa", tone: "border-indigo-400/25 bg-indigo-500/10 text-indigo-100", items: exportDiagnostics.auto_fixed },
