"""OpenAIVideoBackend — backend tạo video OpenAI Sora."""
...
class OpenAIVideoBackend:
    ...
    async def generate(self, request: VideoGenerationRequest) -> VideoGenerationResult:
        ...
        logger.info("Bắt đầu tạo video OpenAI: model=%s, seconds=%s", self._model, kwargs["seconds"])

        video = await self._client.videos.create_and_poll(**kwargs)

        if video.status == "failed":
            raise RuntimeError(f"Tạo video Sora thất bại: {video.error}")

        content = await self._client.videos.download_content(video.id)
        request.output_path.parent.mkdir(parents=True, exist_ok=True)
        request.output_path.write_bytes(content.content)

        logger.info("Đã tải video OpenAI xong: %s", request.output_path)
